...
static CLIENTS: phf::Map<&'static str, ClientDef> = phf::phf_map! {
    ...
    "https://open.spotify.com/track/1VLZs2A4GUrd8jCZJRiTzb" => ClientDef {
        redirect_uris: &[
            "http://localhost:8787/api/login/oauth/cb",
            "https://beetroot.farm/api/login/oauth/cb",
        ],
    }
};
...
pub fn found_redirect(location: &str) -> Response {
    #[expect(
        clippy::unwrap_used,
        reason = "we didn't poison this builder, but checking can't hurt (instead of unwrap_unchecked)"
    )]
    Response::builder()
        .header(
